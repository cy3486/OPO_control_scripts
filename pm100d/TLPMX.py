...
TLPM_STATBIT_OPER_TRIG = (0x0020)  # < The instrument is in a 閿熺弹ait for trigger閿燂拷 state of the trigger model.
TLPM_STATBIT_OPER_ARM = (0x0040)  # < The instrument is in a 閿熺弹ait for arm閿燂拷 state of the trigger model.
...
class TLPMX:
	...
	def writeRegister(self, reg, value):
		...
		Args:
			...
			STATUS BYTE bits (see IEEE488.2-1992 鎼�11.2)
			TLPM_STATBIT_STB_AUX        (0x01): Auxiliary summary
			TLPM_STATBIT_STB_MEAS       (0x02): Device Measurement Summary
			TLPM_STATBIT_STB_EAV        (0x04): Error available
			TLPM_STATBIT_STB_QUES       (0x08): Questionable Status Summary
			...
			STANDARD EVENT STATUS REGISTER bits (see IEEE488.2-1992 鎼�11.5.1)
			TLPM_STATBIT_ESR_OPC        (0x01): Operation complete
			TLPM_STATBIT_ESR_RQC        (0x02): Request control
			TLPM_STATBIT_ESR_QYE        (0x04): Query error
			TLPM_STATBIT_ESR_DDE        (0x08): Device-Specific error
			...
			QUESTIONABLE STATUS REGISTER bits (see SCPI 99.0 鎼�9)
			TLPM_STATBIT_QUES_VOLT      (0x0001): Questionable voltage measurement
			TLPM_STATBIT_QUES_CURR      (0x0002): Questionable current measurement
			TLPM_STATBIT_QUES_TIME      (0x0004): Questionable time measurement
			TLPM_STATBIT_QUES_POW       (0x0008): Questionable power measurement
			...
			OPERATION STATUS REGISTER bits (see SCPI 99.0 鎼�9)
			TLPM_STATBIT_OPER_CAL       (0x0001): The instrument is currently performing a calibration.
			TLPM_STATBIT_OPER_SETT      (0x0002): The instrument is waiting for signals to stabilize for measurements.
			TLPM_STATBIT_OPER_RANG      (0x0004): The instrument is currently changing its range.
			TLPM_STATBIT_OPER_SWE       (0x0008): A sweep is in progress.
			TLPM_STATBIT_OPER_MEAS      (0x0010): The instrument is actively measuring.
			TLPM_STATBIT_OPER_TRIG      (0x0020): The instrument is in a 閳ユ辅ait for trigger閳ワ拷 state of the trigger model.
			TLPM_STATBIT_OPER_ARM       (0x0040): The instrument is in a 閳ユ辅ait for arm閳ワ拷 state of the trigger model.
			TLPM_STATBIT_OPER_CORR      (0x0080): The instrument is currently performing a correction (Auto-PID tune).
			TLPM_STATBIT_OPER_SENS      (0x0100): Optical powermeter sensor connected and operable.
			TLPM_STATBIT_OPER_DATA      (0x0200): Measurement data ready for fetch.
			TLPM_STATBIT_OPER_THAC      (0x0400): Thermopile accelerator active.
	...
	def setZeroPos(self, zeroX, zeroY, channel):
		...
		Args:
			zeroX(c_double) : This parameter set the zero x in 纰宮.
			zeroY(c_double) : This parameter set the zero y in 纰宮.
			channel(c_uint16) : Number of the sensor channel. 
	...
	def getZeroPos(self, pZeroX, pZeroY, channel):
		...
		Args:
			pZeroX(c_double use with byref) : This parameter returns the zero x value in 纰宮.
			pZeroY(c_double use with byref) : This parameter returns the zero y value in 纰宮.
			channel(c_uint16) : Number of the sensor channel. 
	...
	def setExtNtcParameter(self, r0Coefficient, betaCoefficient, channel):
		...
		Args:
			r0Coefficient(c_double) : This parameter specifies the R0 coefficient in [Ohm] for calculating the temperature from the sensor's resistance by the beta parameter equation. R0 is the NTC's resistance at T0 (25 鎺矯 = 298.15 K).
	...
	def getPositionAnalogOutputSlopeRange(self, minSlope, maxSlope, channel):
		"""
		This function returns range of the responsivity in volts per 纰宮 [V/纰宮] for the analog output.
		...
		Args:
			minSlope(c_double use with byref) : This parameter returns the minimum slope in [V/纰宮] of the analog output.
			
			maxSlope(c_double use with byref) : This parameter returns the maximum slope in [V/纰宮] of the analog output.
	...
	def setPositionAnalogOutputSlope(self, slope, channel):
		"""
		This function sets the responsivity in volts per 纰宮 [V/纰宮] for the analog output.
		...
		Args:
			slope(c_double) : This parameter specifies the responsivity in volts per 纰宮 [V/纰宮] for the AO2 and AO3 channel 
	...
	def getPositionAnalogOutputSlope(self, attribute, slope, channel):
		"""
		This function returns the responsivity in volts per 纰宮 [V/纰宮] for the analog output channels.
		...
		Args:
			...
			slope(c_double use with byref) : This parameter returns the specified responsivity in volts per 纰宮 [V/纰宮] for the AO2 and AO3 channel 
	...
	def measPowerDens(self, powerDensity, channel):
		...
		Args:
			powerDensity(c_double use with byref) : This parameter returns the actual measured power density in watt per square centimeter [W/cm铏廬.
	...
	def measEnergyDens(self, energyDensity, channel):
		...
		Args:
			energyDensity(c_double use with byref) : This parameter returns the actual measured energy in joule per square centimeter [J/cm铏廬.
	...
	def measEmmTemperature(self, temperature, channel):
		...
		Args:
			temperature(c_double use with byref) : This parameter returns the temperature in 鎺矯
			channel(c_uint16) : Number of the sensor channel. 
	...
	def measHeadTemperature(self, frequency, channel):
		...
		Args:
			frequency(c_double use with byref) : This parameter returns the temperature in 鎺矯
			channel(c_uint16) : Number of the sensor channel. 
	...
	def meas4QPositions(self, xPosition, yPosition, channel):
		...
		Args:
			xPosition(c_double use with byref) : This parameter returns the actual measured x position in 纰宮
			yPosition(c_double use with byref) : This parameter returns the actual measured y position in 纰宮
			channel(c_uint16) : Number of the sensor channel. 
	...
	def measNegPulseWidth(self, negativePulseWidth, channel):
		"""
		This function returns the negative pulse width in 纰宻ec.
		Notes:
		(1) The function is only available on PM103.
		
		
		Args:
			negativePulseWidth(c_double use with byref) : Negative Pulse Width in 纰宻ec.
			channel(c_uint16) : Number of the sensor channel. 
	...
	def measPosPulseWidth(self, positivePulseWidth, channel):
		"""
		This function returns the positive pulse width in 纰宻ec.
		Notes:
		(1) The function is only available on PM103.
		
		
		Args:
			positivePulseWidth(c_double use with byref) : Positive Pulse Width in 纰宻ec.
			channel(c_uint16) : Number of the sensor channel. 
	...
	def measPowerMeasurementSequence(self, baseTime, channel):
		...
		Args:
			baseTime(c_uint32) : interval between two measurements in the array in 纰宻ec.
	...
	def measPowerMeasurementSequenceHWTrigger(self, baseTime, hPos, channel):
		...
		Args:
			...
			interval between two measurements in the array in 纰宻ec. The maximum resolution is defined in the device specifications..
	...
	def measureCurrentMeasurementSequence(self, baseTime, channel):
		...
		Duration of measurement in 纰宻ec = Count* Interval
	...
	def getNextFastArrayMeasurementRelativeTime(self, count, timestamps, values, channel):
		...
		This function is used to obtain measurements from the instrument. 
		The result are timestamp - value pairs.
		The timestamps are relative to the first timestamp in 纰宻ec.
		...
		Args:
			...
			The value will be 200
			timestamps( (c_uint32 * arrayLength)()) : Buffer containing up to 200 timestamps.
			This are timestamps in 纰宻ec relative to the first timestamp.
			...
			means that the time difference between the samples are 10 纰宻ec.
	...
	def getMeasurementSequence(self, baseTime, timeStamps, values, values2):
		...
		Args:
			...
			Every sample is 10纰宻 for PM5020.
	...
	def confBurstArrayMeasTrigger(self, trgSource, initDelay, burstCount, averaging):
		...
		All entered values are multiplied internally by 10纰宻. The sum of all values is the burst sequence time.
		
		E.g. 
		Init Delay 5
		Burst Count 2
		Averaging 3
		=> Burst Sequence time = (InitDelay + (Burst Count * Averaging))*10纰宻 = (5+2*3)*10纰宻 =110纰宻
		...
		Args:
			...
			initDelay(c_uint32) : Init Delay time in 10纰宻.
			E.g. 5 means 5*10纰宻 = 50纰宻 
			burstCount(c_uint32) : Enter the Sequence Count in 10纰宻 for the burst sequence time.
			E.g. Sequence count = 2: Burst Sequence time = (InitDelay + (SequenceCount * Averaging)) * 10纰宻
			averaging(c_uint32) : Average time in 10纰宻
			E.g. Entered 3 means averaging of 30纰宻
		Returns:
			int: The return value, 0 is for success
		"""
