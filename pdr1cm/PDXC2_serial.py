...
def main():    
    ...
    try:
        ...
        if channelStatus == 0:
            ...
            OpenLoopMove(ser)
            OpenLoopMoveTo(ser, step_size, target_pos)

            

            # The close loop mode is only valid for PDX series stages with encoder
            # CloseLoopMove(ser)  # cy3486 made this into a comment 
...
def OpenLoopMoveTo(ser, step_size, target_pos):
    global openLoopStep
    openLoopStep = step_size
    moves = target_pos // step_size
    for _ in range(moves):
        result = OpenLoopMove(ser)
        if result != 1:
            print("Move failed.")
            break
